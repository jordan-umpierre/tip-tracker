...
CREATE TABLE federal_withholding_settings (
  ...
  -- Settings are append-only in the current UI, but sync has to retain a
  -- server deletion long enough for every offline device to observe it.
  deleted_at TEXT,
...
-- Sync bookkeeping is device state, not another copy of the financial data.
-- One row binds this database to at most one cloud account and stores the last
...
-- This is a compact dirty set. Replacing an entry allocates a newer sequence,
-- so acknowledging an in-flight change cannot erase a later local edit.
...
CREATE TRIGGER jobs_sync_insert
...
BEGIN
  ...
  VALUES ('job', NEW.id, 'upsert');
END;

CREATE TRIGGER jobs_sync_update
...
CREATE TRIGGER jobs_sync_delete
...
BEGIN
  ...
  INSERT INTO sync_outbox (entity_type, entity_id, operation)
  VALUES ('job', OLD.id, 'delete');
END;

CREATE TRIGGER shifts_sync_insert
...
BEGIN
  ...
  INSERT INTO sync_outbox (entity_type, entity_id, operation)
  VALUES ('shift', NEW.id, 'upsert');
END;

CREATE TRIGGER shifts_sync_update
...
CREATE TRIGGER shifts_sync_delete
...
BEGIN
  ...
  INSERT INTO sync_outbox (entity_type, entity_id, operation)
  VALUES ('shift', OLD.id, 'delete');
END;

CREATE TRIGGER federal_settings_sync_insert
AFTER INSERT ON federal_withholding_settings
FOR EACH ROW
WHEN (SELECT applying_remote FROM sync_state WHERE singleton = 1) = 0
BEGIN
...
AFTER UPDATE ON federal_withholding_settings
FOR EACH ROW
WHEN (SELECT applying_remote FROM sync_state WHERE singleton = 1) = 0
BEGIN
  DELETE FROM sync_outbox
...
AFTER DELETE ON federal_withholding_settings
FOR EACH ROW
WHEN (SELECT applying_remote FROM sync_state WHERE singleton = 1) = 0
BEGIN
  DELETE FROM sync_outbox
