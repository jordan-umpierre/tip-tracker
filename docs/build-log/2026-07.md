...
Archived out of `BUILD_LOG.md` on 2026-07-30, when that file passed the
~500 line split threshold. Split by calendar month, same rule and same
reason as `docs/brainstorm/`'s Q&A archive: a numbered sequel file makes
things unfindable, a date doesn't.

Companion docs: `../../BUILD_LOG.md` for the index and why this file exists
at all, `../../DECISIONS.md` for the numbered decisions referenced below,
`../../BRAINSTORM.md` for product thinking and the Order of Operations.
