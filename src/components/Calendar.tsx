...
// The calendar itself, with no opinion about what contains it. CalendarPicker
// puts it in a bottom sheet for the forms that ask for a date in passing; the
// log-a-shift date step puts it straight on a screen of its own.
//
// It pages by the full window width, so whatever renders it has to let it run
// edge to edge. Anything narrower and the month panes stop lining up with the
// swipe.
type Props = {
  // The date the calendar opens on, as YYYY-MM-DD. Also the cell drawn as
  // selected, so an unparseable value just means nothing looks selected rather
  // than the calendar refusing to render -- a field it comes from may let the
  // user type freely.
  selectedDate: string;
...
export default function Calendar({ selectedDate, datesWithShifts, onSelect }: Props) {
